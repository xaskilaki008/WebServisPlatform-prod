...
const operatorCategoryValue = document.getElementById('operator-category-value');
const operatorDirectionValue = document.getElementById('operator-direction-value');
...
function updateDetailScreen(beach = {}) {
    ...
    detailWaveLevel.textContent = beach.effective_wave_level ?? beach.wave_level ?? '-';
...
function openBeachPopup(marker, beach) {
    const hideNumber = hidePopupNumberOnce;
    hidePopupNumberOnce = false;
    marker.setPopupContent(buildPopupContent(beach, { hideNumber }));
    marker.openPopup();
...
function setMapExpanded(nextState) {
    ...
    const icon = isMapExpanded
        ? '/значки%20и%20иконки/map-controls/collapse-map.png'
        : '/значки%20и%20иконки/map-controls/expand-map.png';
    toggleMapSizeButton.title = label;
